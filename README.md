...
| [ejentum-mcp](https://github.com/ejentum/ejentum-mcp) | MCP server exposing four cognitive harness tools (`harness_reasoning`, `harness_code`, `harness_anti_deception`, `harness_memory`) OpenClaw agents call before generating. Each tool returns a structured prompt: a named failure pattern, an executable procedure, suppression vectors that block common shortcuts, and a falsification test for self-verification. Targets hallucinated APIs, sycophantic capitulation, false-confidence validation, and cross-turn drift on long agentic sessions. stdio via `npx -y ejentum-mcp`, requires `EJENTUM_API_KEY` (free tier available at ejentum.com), MIT licensed, listed on the Official MCP Registry as `io.github.ejentum/ejentum-mcp`. |
